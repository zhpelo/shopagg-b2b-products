...
4. **设置"产品展示页面"** - 选择包含 `[b2b_products]` 简码的页面
5. 点击"保存设置"
...
2. 点击"添加新分类"
3. 填写分类名称（必填）
4. 选择父分类（可选，留空则创建顶级分类）
5. 填写分类描述（可选）
6. 填写分类别名（可选，不填写将自动生成）
7. 设置排序（数字越小越靠前）
8. 点击"添加分类"
...
| `pagination` | 字符串 | `yes` | 是否启用分页（yes/no/true/false/1/0） |
| `show_filters` | 字符串 | `yes` | 是否显示分类筛选器（yes/no/true/false/1/0） |

### [b2b_products_categories] 参数

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `show_count` | 字符串 | `yes` | 是否显示产品数量（yes/no） |
| `show_description` | 字符串 | `no` | 是否显示分类描述（yes/no） |
| `expand_all` | 字符串 | `no` | 是否默认展开所有分类（yes/no） |
| `link_to_products` | 字符串 | `yes` | 是否链接到产品页面（yes/no） |
...
### 分类树相关类
- `.b2b-categories-tree-wrapper` - 分类树容器
- `.b2b-categories-tree` - 分类树列表
- `.b2b-category-item` - 分类项
- `.b2b-category-toggle` - 展开/折叠按钮
- `.b2b-category-link` - 分类链接
- `.b2b-category-count` - 产品数量

### 产品详情页相关类
- `.b2b-product-detail-wrapper` - 详情页容器
- `.b2b-product-detail-main-image` - 主图片区域
- `.b2b-product-nav-btn` - 左右切换按钮
- `.b2b-product-detail-thumbnails` - 缩略图列表
- `.b2b-product-thumbnail` - 单个缩略图
